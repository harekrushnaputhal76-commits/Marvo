  ...
  const StudyModeUI = {
     ...
     */
     enter() {
       ...
       // Initialize / start Gyroscope Parallax for Study Mode Cards
       try {
         const EngineClass = window.TiltEngine || (typeof TiltEngine !== 'undefined' ? TiltEngine : null);
         if (EngineClass) {
           if (!this.tiltEngine) {
             this.tiltEngine = new EngineClass({
               maxTilt: 14,
               smoothing: 6,
               fps: 30,
               idleMs: 1500,
               onUpdate: ({ pitch, roll }) => {
                 ...
                 const studyCards = document.querySelectorAll(
                   '.study-hero-banner, .ocr-card, .flashcard-card-3d, .quiz-card, .flashcard-inner'
                 );
                 if (studyCards.length > 0 && typeof window.applyTilt === 'function') {
                   studyCards.forEach(c => window.applyTilt(c, pitch * 0.75, roll * 0.75));
                 }
               }
             });
           }
         ...
         console.warn('[StudyMode] TiltEngine initialization error:', tiltErr);
       }
     },
